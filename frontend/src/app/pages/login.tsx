  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 403 && error.response?.data?.requiresOTP) {
        toast.info("Verification required. OTP sent.");
        setShowOTP(true);
        return;
      }
      ...
      <div className={`hidden lg:flex flex-col justify-between w-[40%] p-12 relative overflow-hidden`}>
        ...
        <div className="relative z-10 max-w-lg mb-12">
          <h2 className="text-6xl font-extrabold text-white leading-[1.1] mb-8">
            Everything you <br />
            need to <span className={`bg-clip-text text-transparent bg-gradient-to-r ${primaryGradient} animate-gradient-x`}>succeed</span>
            <br />in one place.
          </h2>
          <div className="space-y-8 mt-12">
            {[
              { icon: "✨", title: "Premium Experience", desc: "A sleek, modern interface designed for focus and productivity." },
              { icon: "🤝", title: "Smart Collaboration", desc: "Connect with the best talent or find the perfect project." },
              { icon: "⚡", title: "Instant Actions", desc: "Real-time notifications and lightning-fast workflows." },
            ].map((item, i) => (
        ...
        <div className="relative z-10 pt-10 border-t border-white/10">
          <p className="text-slate-400 text-sm font-medium italic">
            "The most beautiful platform I've ever used. Simply stunning."
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden">
        <div className="h-full w-full flex flex-col items-center justify-center p-4 lg:p-8 relative">
          ...
          <div className="w-full max-w-[480px] relative z-10 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] transform transition-all duration-500 overflow-hidden group">
            ...
            {/* Unique Fading Scroll Effect Container */}
            <div className="relative h-[650px] overflow-y-auto no-scrollbar scroll-smooth p-6 lg:p-10">
              {/* Top Fade Mask */}
              <div className="absolute top-0 left-0 right-0 h-10 bg-gradient-to-b from-[#0f172a] to-transparent z-20 pointer-events-none sticky top-0" />

              {/* Bottom Fade Mask */}
              <div className="absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-[#0f172a] to-transparent z-20 pointer-events-none sticky bottom-0" />
                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center ml-1">
                        ...
                        <a href="#" className={`text-[10px] font-black uppercase tracking-widest transition-colors ${userType === 'freelancer' ? 'text-emerald-500 hover:text-emerald-400' : 'text-indigo-400 hover:text-indigo-300'}`}>
                          FORGOT?
                        </a>
                      </div>
                      <div className="relative group">
            ...
            {isLoading && (
              <div className="absolute inset-0 z-50 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center rounded-[32px] animate-in fade-in duration-300">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-white font-black text-sm tracking-widest animate-pulse">SECURING ACCESS...</p>
                </div>
              </div>
            )}
          </div>
